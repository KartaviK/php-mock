 ...
 */
class FixedMicrotime implements Contract\Functions\ProviderInterface, Contract\Functions\IncrementInterface
{
     ...
     */
    public function setMicrotime(string $timestamp): void
    {
        if (empty($timestamp)) {
     ...
     *
     * @param bool $returnAsFloat If true returns timestamp as float, else string
     *
     * @return mixed The value.
     * @SuppressWarnings(PHPMD)
     */
    public function getMicrotime($returnAsFloat = false)
    {
        if ($returnAsFloat) {
            $converter = new MicrotimeConverter();
